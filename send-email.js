import { requireAdmin, jsonResponse } from './_verify-helper.js';
...
export async function onRequestPost({ request, env }) {
  ...
  if (!ALLOWED_TYPES.includes(type)) return jsonResponse({ error: 'Invalid email type' }, 400);
  if (!to || !to.includes('@')) return jsonResponse({ error: 'Valid recipient email required' }, 400);
  if (!order_ref) return jsonResponse({ error: 'order_ref required' }, 400);
  ...
  try {
    ...
    if (!resp.ok) return jsonResponse({ error: 'Resend error', detail: data }, 502);
    return jsonResponse({ success: true, id: data.id });
  } catch (err) {
    return jsonResponse({ error: 'Failed to send email', detail: String(err) }, 502);
  }
}
